...
pub fn init_block(ptr : *mut u32, idx : usize) -> bool {
    unsafe {
        ...
        let queue_ptr = alloc_kernel_page(num_pages).unwrap() as *mut Queue;
        // 8. 设置页面大小
		let queue_pfn = queue_ptr as u32;
		ptr.add(Offset::GuestPageSize.scale32()).write_volatile(PAGE_SIZE as u32);
		// 9. 设置 Queue 所在页面号码
		ptr.add(Offset::QueuePfn.scale32()).write_volatile(queue_pfn / PAGE_SIZE as u32);
		// 10. 设置 DRIVER_OK
		status_bits |= StatusField::DriverOk.val32();
		ptr.add(Offset::Status.scale32()).write_volatile(status_bits);
...
use crate::{memory::{alloc, alloc_kernel_page, config::PAGE_SIZE, free}, sync::Bool, uart};
use crate::libs::syscall::branch;