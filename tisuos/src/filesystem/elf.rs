...
pub fn load_elf(elf : &mut ElfManager) {
    unsafe{
        assert!(elf.elf.is_elf());
        let num = elf.elf.program_header_num as usize;
        let entry = elf.elf.entry as usize;
        // let id = exec(entry);
        let mgr = crate::task::get_task_mgr().unwrap();
        let id = mgr.create_task(entry, false).unwrap();
        for i in 0..num{
            let program = elf.next_ph().unwrap();
            if program.is_loadable(){
                println!("id {}, loadable", i);
                ...
                let physic_addr = get_manager().user_page(num_page).unwrap() as usize;
                let buffer = elf.get_addr(program.offset());
                buffer.copy_to(physic_addr as *mut u8, file_size);
                for n in 0..num_page{
...
use crate::memory::{config::PAGE_SIZE, get_manager};