...
pub fn load_elf(file : File)->Option<(usize, usize)>{
    unsafe{
        let mut file = file;
        if file.is_close() && file.open(OpenFlag::Read.val()).is_err(){
            return None;
        }
        let head;
        ...
        let elf = &*(head.addr as *const ELF);
        if elf.is_elf(){
            ...
            for i in 0..num{
                ...
                if let Some(phr) = file.read(offset, phsize){
                    let program = &*(phr.addr as *const ProgramHeader);
...
use crate::{memory::{page::{PAGE_SIZE, alloc_user_page}, page_table::{self, PageTable}}, uart};

use super::file::{File, OpenFlag};