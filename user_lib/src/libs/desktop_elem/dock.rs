    ...
    pub fn new(ftype : DockFuncType)->Self {
        ...
        match ftype {
            DockFuncType::OpenFileWindow | DockFuncType::TriggerFileWindow(_) => {
                path = "1/img/folder.bmp".to_string();
            }
        ...
        let ptr = file_info(path) as *const FileInfo;
        let info = unsafe {&*(ptr)};
        let data = Block::<u8>::new(info.size);
        let image = BMP::decode(data.to_array(0, info.size)).unwrap();
...
use crate::{Area, HEIGHT, Position, WIDTH, file_info, libs::{graphic::ColorBlock, memory_block::Block}, open};
